...
2. 非阻塞I/O模型：调用recvfrom，如果该缓存区没有数据的话，直接返回EWOULDBLOCK错误（recvfrom系统调用flags的传参使用MSG_DONTWAIT）。通过轮询检查这个状态，看内核是不是有数据到来（一直返回EWOULDBLOCK，说明没有数据）。
3. I/O复用模型：Linux提供select/poll，进程通过将一个或多个fd（file descriptor，文件描述符）传递给select或poll系统调用，阻塞在select操作上，这样select/poll可以帮我们侦测多个fd是否处于就绪状态（顺序扫描，支持的fd数量有限）。除此之外，Linux还提供了epoll调用，epoll使用基于事件驱动方式代替顺序扫描，因此性能更高。当有fd就绪时，立即回调函数。
