...
> **局部性原理（Locality Principle）** 是计算机科学中的核心理论，描述了程序在访问数据或指令时呈现的集中倾向性规律。它是计算机体系结构、操作系统、数据库设计等领域性能优化的基础，尤其在缓存设计中至关重要。其核心思想是：**程序倾向于重复使用最近访问过的数据或指令，以及访问附近的数据。**
>
> **局部性原理的两大类型**：
> 1. 时间局部性：如果一个数据/指令被访问过，它在不久的将来很可能被**再次访问**。
> 2. 空间局部性：如果一个数据被访问，其**邻近地址的数据**很可能在不久后被访问。
>
> **应用**：
> - 索引设计：B+树索引的叶子节点通过双向链表连接，利用空间局部性加速范围查询。
> - 缓冲池（Buffer Pool）：缓存热点数据页（时间局部性），减少磁盘I/O。
> - 预读机制（Read-Ahead）：顺序扫描表时预加载相邻数据页（空间局部性）。
...
## 缓冲池污染
当发生读取大量数据时，缓冲池中的页可能全部被替换出去，导致大量热数据被换出，使MySql性能急剧下降，这种情况称为缓冲池污染。
...
因此，MySQL的缓冲池列表使用了变种的LRU算法进行管理。当需要将新页添加到缓冲池时，最近最久未使用的页会被淘汰，新页被添加到列表中间。如下图所示这种中点插入策略将列表视为两个子列表。
...
2. innodb_old_blocks_pct：老年代占整个LRU链表长度的比例，默认是37。
