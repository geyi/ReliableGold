...
在iptables/IPVS 模式下，kube-proxy实际不自己处理流量，流量完全由内核处理。kube-proxy只监听service的变化，并更新规则。这样既避免了单点的性能瓶颈，又提高了可靠性（因为流量无需经过 kube-proxy 进程，并且 kube-proxy 进程崩溃，已有的网络规则仍能正常工作）。
