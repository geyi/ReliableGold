...
早期，CPU通过在总线上加LOCK#锁来解决缓存不一致的问题。因为CPU和其他部件通信都要通过总线，如果对总线加LOCK#锁的话，也就阻塞了其他CPU对其他部件的访问（如内存），此时只能有一个CPU使用这个变量的内存。在如上例子中，一个线程在执行 i = i + 1前，在总线上发出了LOCK#锁的信号，那么只有等待这段代码完全执行完毕之后并释放锁，其他CPU才能从变量i所在的内存读取i的值，然后进行相应的操作，这样就解决了缓存不一致的问题。
