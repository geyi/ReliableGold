...
两个线程同时执行如上代码，假设初始时i的值是0，那么我们期望两个线程执行完后i的值为2。但结果一定是2吗？

存在如下情况：两个线程分别读取i的值到各自的CPU高速缓存中（此时i的值为0），然后线程A对i执行加1操作，并将结果写入到内存中。此时内存中i的值为1，但在线程B的高速缓存中i的值仍然是0，线程B同样执行加1操作，并向内存中回写i的值。最终内存中i的值仍然为1。
...
在多处理器系统中，总线锁（Bus Locking）是一种确保对共享资源（如内存）进行原子操作的硬件机制。总线锁通过锁住内存总线，防止其他处理器访问某些共享内存区域，从而确保数据一致性和操作的原子性。

虽然总线锁能够有效地解决同步问题，但它也有一个很明显的缺点，由于在锁住总线期间，其他CPU无法访问内存，导致效率低下。

在现代处理器和系统设计中，更多地采用了缓存一致性协议和细粒度锁机制来替代传统的总线锁，缓存一致性协议保证了每个缓存中使用的共享变量的副本是一致的。它核心的思想是：当CPU写数据时，如果发现操作的变量是共享变量，即在其他CPU中也存在该变量的副本，会发出信号通知其他CPU将该变量的缓存行置为无效状态，因此当其他CPU需要读取这个变量时，发现自己缓存中该变量的缓存行是无效的，那么它就会从内存重新读取。
