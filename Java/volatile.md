...
两个线程同时执行如上代码，假如初始时i的值是0，那么我们期望两个线程执行完后i的值为2。但结果一定是2吗？

存在如下情况：两个线程分别读取i的值到各自的CPU高速缓存中（此时i的值为0），然后线程A对i执行加1操作，并将结果写入到内存中。但在线程B的高速缓存中i的值仍然是0，执行加1操作，i的值为1，然后线程B再把i的值写入内存。最终i的值为1。 
