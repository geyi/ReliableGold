...
**进程**：是指操作系统中正在运行的一个程序的实例。每个进程都有自己的地址空间、内存和资源。一个进程可以包含多个线程，它们共享相同的内存空间和资源。进程之间是相互独立的，每个进程在执行过程中都有自己的状态、堆栈和上下文切换开销。（动态）。  
**线程**：是指进程中的一个执行单元。一个进程可以包含多个线程，线程与进程共享相同的内存空间和资源。线程是并发执行的，它可以独立地执行任务，但仍然受到进程的管理和控制。线程之间可以共享数据和上下文，线程的创建和销毁的开销较小。
