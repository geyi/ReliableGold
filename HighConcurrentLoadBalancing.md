# OSI七层参考模型
应用层：应用层直接与应用程序对接提供常见的网络服务  
表示层：协议、语义、段落的划分，字符串的表示、加密  
会话层：session，会话的保持  
传输层：如何建立连接，如何传输，应该是成功的还是失败  
网络层：如何路由，如何找到对应的节点，如何通信，数据包应该如何发送  
数据链路层：点与点之间  
物理层：光纤、WiFi
...
192.168.1.0     0.0.0.0         255.255.255.0   U     100    0        0 ens33
```
代表ens33网卡是在192.168.1.0局域网中的，而同一局域网中的两台主机之间的通信不需要下一跳转发（网与网之间才需要下一跳的转发）

同一局域网不需要网关，不需要下一跳。直接通过交换机转发，不需要走路由。 所以Gateway 0.0.0.0表示当前设备在Destination中有一个具体的ip。 

# 链路层
网络层找到下一跳的地址是192.168.1.1，但是实际要访问的地址是14.215.177.39（百度），那么数据是如何发送给192.168.1.1的？
...
# 总结
TCP/IP协议是基于下一跳机制，IP地址是端点间（两台主机），MAC地址是节点间。
...
基于3层，带宽成为瓶颈（下行成为瓶颈），消耗算力（来回都要处理）。  
