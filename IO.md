...
同步：在一次执行过程中需要等待某一个操作执行完成才能继续执行后面的操作则这个操作是同步的。在网络I/O中同步的体现是应用程序自己R/W。  
异步：内核完成R/W。  
阻塞与非阻塞：阻塞与非阻塞说的是被调用的方法。方法立即返回，通常利用一个回调来处理之后产生的结果，这个方法称为非阻塞的。

同步阻塞：程序自己读取，调用方法一直等待有效返回结果。
同步非阻塞：程序自己读取，调用方法立即返回是否读到。
...
> 如果内核在回调处理中加入--除了将数据写入与fd关联的buffer中，同时将这个fd移动到另外一个列表中，当应用程序调用epoll_wait时，直接从列表中获取准备好的fd。
